...
import { LabelAIRecipeSection } from "./LabelAIRecipeSection";
import { LabelGummySpecSection } from "./LabelGummySpecSection";
...
export const EditLabelModal = ({ open, onClose, label, onSuccess }: EditLabelModalProps) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData.append("flavorComponents", JSON.stringify(
        flavorComponents.map((c) => ({ name: c.name.trim(), percentage: Number(c.percentage) }))
      ));
      formData.append("colorComponents", JSON.stringify(
  ...
  const inputCls = "rounded-xs border-border dark:border-white/20 bg-card";
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto scrollbar-hidden rounded-xs border-border dark:border-white/20 dark:bg-card">
        ...
        <div className="space-y-4">
          <div>
            ...
            <Input id="flavorName" placeholder="e.g., Mango Blast" value={flavorName}
              onChange={(e) => setFlavorName(e.target.value)} className={inputCls} />
          </div>

          <div>
            <Label htmlFor="productType">Product Type *</Label>
            <Select value={productType} onValueChange={setProductType}>
              <SelectTrigger id="productType" className={inputCls}>
                <SelectValue placeholder="Select product type" />
              </SelectTrigger>
              <SelectContent className={inputCls}>
          ...
          <div>
            ...
            <Input id="cannabinoidMix" placeholder="e.g., 10mg THC BIOMAX" value={cannabinoidMix}
              onChange={(e) => setCannabinoidMix(e.target.value)} className={inputCls} />
          </div>

          <div>
            <Label htmlFor="color">Color</Label>
            <Input id="color" placeholder="e.g., Red/Pink" value={color}
              onChange={(e) => setColor(e.target.value)} className={inputCls} />
          </div>

          <LabelAIRecipeSection
            selectedFlavors={selectedFlavors}
            onSelectedFlavorsChange={setSelectedFlavors}
            gummyColorHex={gummyColorHex}
            onGummyColorHexChange={setGummyColorHex}
            gummyColorName={gummyColorName}
            onGummyColorNameChange={setGummyColorName}
            onColorChange={setColor}
            allFlavors={allFlavors}
            isLoadingFlavors={isLoadingFlavors}
          />

          <LabelGummySpecSection
            gummySize={gummySize}
            onGummySizeChange={setGummySize}
            gummyOilType={gummyOilType}
            onGummyOilTypeChange={setGummyOilType}
            gummyEffect={gummyEffect}
            onGummyEffectChange={setGummyEffect}
            gummyCannabinoids={gummyCannabinoids}
            onGummyCannabinoidsChange={setGummyCannabinoids}
            unitsOrdered={unitsOrdered}
            onUnitsOrderedChange={setUnitsOrdered}
            unitCost={unitCost}
            totalCost={totalCost}
          />
          ...
          <LabelComponentList label="Flavor Components" items={flavorComponents} onChange={setFlavorComponents} />
          <LabelComponentList label="Color Components" items={colorComponents} onChange={setColorComponents} />
          ...
          <div className="flex justify-end gap-2 pt-4">
            <Button variant="outline" onClick={onClose} disabled={isLoading} className={inputCls}>
              Cancel
            </Button>
