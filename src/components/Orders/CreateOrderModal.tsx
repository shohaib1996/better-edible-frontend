...
export const CreateOrderModal = ({ open, onClose, store }: CreateOrderModalProps) => {
  ...
  const handleSubmit = async () => {
    if (!store?._id) { toast.error("Store information is missing"); return; }
    if (!repId) { toast.error("Please select a rep"); return; }
    if (orderItems.length === 0) { toast.error("Please add at least one product to the order"); return; }
