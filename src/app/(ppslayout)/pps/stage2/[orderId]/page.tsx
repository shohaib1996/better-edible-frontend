    ...
    <Card className="rounded-xs">
      ...
      <CardContent className="flex flex-col gap-3">
        ...
        {item.assignedMoldIds.length > 0 && (
          <div>
            <p className="text-xs text-muted-foreground mb-1.5">Assigned Molds</p>
              ...
              {item.assignedMoldIds.map((id) => {
                ...
                return (
                  <Badge
                    ...
                    className={`text-xs font-mono ${isLoaded ? "bg-green-600 text-white" : ""}`}
                  >
                    {isLoaded ? "✓ " : ""}{id}
                  </Badge>
                );
  ...
  const unprocessedPairs: { cookItemId: string; moldId: string; flavor: string }[] =
    orderItems.flatMap((item) => {
      ...
      return item.assignedMoldIds
        ...
        .map((moldId) => ({ cookItemId: item.cookItemId, moldId, flavor: item.flavor }));
    });
    ...
    <div className="p-4 md:p-8 bg-background flex-1 overflow-y-auto overscroll-contain">
      <div className="flex items-center gap-3 mb-6">
        <Button
          variant="ghost"
          size="icon"
          onClick={() => view === "dehydrator" ? setView("items") : router.push("/pps")}
          className="shrink-0"
        >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold">
              {view === "dehydrator" ? "Load to Dehydrator" : storeName ?? "Stage 2 — Dehydrator Loading"}
            </h1>
            <p className="text-sm text-muted-foreground font-mono">Order {decodedOrderId}</p>
          </div>
        </div>
      </div>
