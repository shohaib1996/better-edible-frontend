...
export default function WorkerStage4ItemPage({
  params,
}: {
...
<body>${el.innerHTML}</body>
  ...
  if (isLoading) {
    return (
      <div className="flex items-center gap-2 text-muted-foreground py-12 justify-center flex-1">
    ...
    return (
      <div className="p-4 flex-1 overflow-y-auto overscroll-contain">
        ...
        <p className="text-muted-foreground">
          Cook item <span className="font-mono text-sm">{decodedCookItemId}</span> was not found in the Stage 4 queue.
        </p>
      </div>
    );
  }
    ...
    <div className="p-4 flex-1 overflow-y-auto overscroll-contain">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex items-center gap-3 min-w-0 flex-1">
          ...
          <div className="min-w-0">
            ...
            <p className="text-lg text-muted-foreground">{item.storeName}</p>
          </div>
        </div>
        <Badge variant="outline" className={`shrink-0 text-base px-3 py-1 ${statusColor}`}>
          {statusLabel}
        </Badge>
      </div>
      ...
      <div className="grid grid-cols-3 gap-0 border rounded-xs divide-x mb-6">
        ...
        <div className="px-3 py-3">
          ...
          <p className="text-2xl font-bold">{Math.ceil(expectedCount / 70)}</p>
        </div>
        <div className="px-3 py-3">
          <p className="text-xs text-muted-foreground uppercase tracking-wide mb-0.5">Est. Cases</p>
          <p className="text-2xl font-bold">{Math.floor(expectedCount / 100) + (expectedCount % 100 > 0 ? 1 : 0)}</p>
        </div>
      </div>
      ...
      {!isDone ? (
        <div className="flex flex-col gap-5 rounded-xs border bg-card p-5">
          ...
          <DigitCounter value={actualCount} onChange={setCount} />
        ...
        <div className="flex flex-col gap-4 rounded-xs border bg-card p-5">
          ...
          <Button
            size="lg"
            variant="outline"
            className="w-full text-2xl h-16 rounded-xs font-bold gap-3"
