...
export const generateClientOrderPackingList = (
  order: IClientOrder,
  ...
  const docDefinition: TDocumentDefinitions = {
    content: [
      {
        text: "CLIENT ORDER PACKING LIST",
        style: "header",
        alignment: "center",
        color: "#2563eb",
      },
      {
        text: order.client?.store?.name || "N/A",
        style: "subheader",
        color: "#0057b7",
        margin: [0, 10, 0, 2],
      },
      ...
      { text: "\n" },
      {
        ...
        layout: {
          fillColor: (rowIndex: number) => {
            if (rowIndex === 0) return "#dbeafe"; // Blue header
            if (
              rowIndex > 0 &&
              order.items?.[rowIndex - 1] &&
              qaChecks[
                `${order.items[rowIndex - 1].productType}-${
                  order.items[rowIndex - 1].flavorName
                }`
              ]
            ) {
