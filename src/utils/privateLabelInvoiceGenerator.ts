...
export const generatePrivateLabelInvoice = (order: IPrivateLabelOrder) => {
  if (!pdfMake) {
    console.error("pdfMake not initialized. Must run on client side.");
    return;
  }

  console.log("Generating Private Label Invoice for order:", order);
  ...
  const docDefinition: TDocumentDefinitions = {
    content: [
      // Header section
      {
        columns: [
          ...
          },
          {
            width: "auto",
            stack: [
              { text: "PRIVATE LABEL INVOICE", style: "headerRight" },
              {
                table: {
                  widths: ["auto", "auto"],
                  body: [
                    ...
                    [
                      ...
                      { text: order._id.slice(-8).toUpperCase() },
                    ],
                  ],
                },
                layout: "noBorders",
      ...
      // Private Label Badge
      {
        text: "🏷️ PRIVATE LABEL ORDER",
        style: "privateLabelBadge",
        alignment: "center",
        margin: [0, 0, 0, 10],
      },

      // Order items - Detailed view with custom labels info
      {
        table: {
          headerRows: 1,
          ...
          body: [
            ...
            ...(order.items || []).map((item, index) => [
              ...
              {
                stack: [
                  { text: item.privateLabelType, bold: true },
                  item.labelImages && item.labelImages.length > 0
                    ? {
                        text: `📎 ${item.labelImages.length} custom label${
                          item.labelImages.length > 1 ? "s" : ""
                        } attached`,
                        fontSize: 8,
                        color: "#ea580c",
                        italics: true,
        ...
        layout: {
          ...
          vLineWidth: () => 0,
          hLineColor: (i: number) => (i === 1 ? "#ea580c" : "#e5e7eb"),
          paddingLeft: () => 8,
          paddingRight: () => 8,
          paddingTop: () => 6,
          paddingBottom: () => 6,
        ...
        columns: [
          { text: "", width: "*" },
          {
            ...
            layout: {
              ...
              vLineWidth: () => 0,
              hLineColor: (i: number) => (i === 2 ? "#ea580c" : "#e5e7eb"),
              paddingLeft: () => 8,
              paddingRight: () => 8,
            },
            margin: [0, 10, 0, 0],
